...
def generateAllTupleOrCombination(length, elementUpperBound, isTupleOrCombination):
	if isTupleOrCombination == TUPLE:
		return list(itertools.product(range(elementUpperBound), repeat=length))
	else:
		return list(itertools.combinations(range(elementUpperBound), length))
	# allPossibility = []
